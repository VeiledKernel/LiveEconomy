...
class MarketGUI(
    ...
    fun rebuildCache(player: org.bukkit.entity.Player) {
        // View is rebuilt fresh on each open — no stale cache
    }

    // ── Menu builder ──────────────────────────────────────────────────────────

    private fun buildMenu(player: Player, categoryId: String,
                          v: MarketView = views.market(player)): LiveMenu {
        val page       = playerPage.getOrDefault(player.uniqueId, 0)
        val items      = v.categoryItems[categoryId] ?: emptyList()
        val maxPage    = ((items.size - 1) / config.itemSlots.size.coerceAtLeast(1)).coerceAtLeast(0)
        val catName    = categoryId
        val alertCount = v.alertCount
        val index      = v.index
        ...
        if (config.buttons.wallet >= 0) {
            menu.setSlot(config.buttons.wallet, itemStack(Material.PLAYER_HEAD) {
                ...
                lore(Theme.SEP, "${Theme.GRAY}Balance  ${v.balanceFormatted}",
